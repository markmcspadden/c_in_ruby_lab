...
class FullTextSearch < Test::Unit::TestCase
  def setup
    ...
    @s1_expected = [[0.46, "johnny appleseed planted many an apple trees and had an apple a day"],
                [0.1875, "the apple doesnt fall far from the tree"]]    
                
    @s2_expected = [[0.5, "i would like an orange tree"]] 
    
    @s3_expected = [[0.375, "i would like an orange tree"],
                    [0.09375, "the apple doesnt fall far from the tree"]]        
  end  
  
  def test_ruby_to_words
    assert_equal ["this", "is", "an", "apple"], RubyFullTextSearchString.new("This is an apple.").to_words
  end
  ...
  def test_c_to_words
    assert_equal ["this", "is", "an", "apple"], CFullTextSearchString.new("This is an apple.").to_words
  end
